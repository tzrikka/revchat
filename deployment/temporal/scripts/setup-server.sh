...
echo "Server is healthy, setting up namespace '${NAMESPACE}' and search attribute..."
...
temporal operator search-attribute create -n ${NAMESPACE} --address ${TEMPORAL_ADDRESS} --name WaitingForSignals --type KeywordList
echo "Custom search attribute 'WaitingForSignals' created"
